...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String urlString) throws IOException {
        URL url;
        //Get the urlString request into a URL
        try {
            url = new URL(urlString);
        } catch (MalformedURLException e) {
            Log.e(LOG_TAG, "Url is wrong!");
            return null;
        }
        //Start an http connection
        HttpURLConnection connection = null;
        InputStream inputStream = null;
        //Prepare connection to get info
        ...
        connection.connect();
        //put the input stream in variable
        inputStream = connection.getInputStream();

        //Input stream conversion
        InputStreamReader fullReader = new InputStreamReader(inputStream);
        BufferedReader reader = new BufferedReader(fullReader);
        //Ready to read lines from BufferedReader reader

        //String builder to hold it all, String to pull pieces.
        StringBuilder fullString = new StringBuilder();
        String lines = reader.readLine();
        while (lines != null) {
            fullString.append(lines);
            lines = reader.readLine();
        }
        //Should have the full json string, convert it to variable
        ...
        try {
            ...
            // Create root JSON object from data
            JSONObject root = new JSONObject(fullString.toString());
            // Extract the array "features" (List of earthquake objects
            JSONArray features = root.getJSONArray("features");
            // Loop through all the earthquakes and extract data
